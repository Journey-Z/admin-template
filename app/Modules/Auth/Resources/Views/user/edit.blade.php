            ...
            <div class="col-md-12">
                <form class="form-horizontal" method="post" enctype="multipart/form-data" id="add_form">
                    ...
                    <div class="form-group clearfix">
                        ...
                        <input type="text" class="form-control" placeholder="{{trans('Auth::auth.username')}}" id="name" name="name" datatype="s4-18" errormsg="昵称至少4个字符，最多18个字符！" nullmsg="请输入昵称！" value="{{$user->name}}"/>
                        </div>
                    </div>
                    <div class="form-group clearfix">
                        <label class="col-sm-2 control-label">
                        ...
                        <div class="col-sm-9 release-input">
                            <input type="text" class="form-control"  placeholder="{{trans('Auth::auth.email')}}" id="email" name="email" datatype="e" errormsg="必须是邮箱地址！" nullmsg="请输入邮箱！" value="{{$user->email}}"/>
                        </div>
                    </div>
                    {{-- <div class="form-group clearfix">
                        <label class="col-sm-2 control-label">
                        ...
                        <div class="col-sm-9 release-input">
                            <select class="form-control"  name="status">
                                @foreach($allStatus as $key => $val)
                                    <option value="{{ $key }}" @if($user->status == $key) selected @endif>{{ trans('Dashboard::common.' . $val)}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                            ...
                            <div class="col-sm-9 release-input select-roles">
                                    <select id="roles" class="form-control select2 " name="roles[]" multiple>
                                        @foreach($roles as $role)
                                            <option value="{{$role->name}}" @if(in_array($role->name, $user->roles()->pluck('name')->toArray())) selected="selected"@endif>{{$role->display_name}}</option>
                                        @endforeach
                                    </select>
                                </div> 
                        </div>                          
